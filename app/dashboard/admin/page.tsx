    ...
    <div className="flex items-center justify-between py-3 border-b border-zinc-100 last:border-0">
      <div>
        ...
        <p className="text-xs text-zinc-400">
          {tx.items.length} item{tx.items.length !== 1 ? "s" : ""} · by {tx.processed_by}
        </p>
      </div>
...
export default function AdminOverviewPage() {
  ...
  const todayItems = todayTxns.reduce((sum, tx) => sum + tx.items.reduce((s, i) => s + i.quantity, 0), 0)
  const recentFive = [...todayTxns].reverse().slice(0, 5)
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-zinc-200 p-5">
            <h2 className="text-sm font-semibold text-zinc-700 mb-3">Today's Transactions</h2>
            {recentFive.length === 0 ? (
              <p className="text-sm text-zinc-400 py-6 text-center">No transactions recorded today</p>
            ) : (
