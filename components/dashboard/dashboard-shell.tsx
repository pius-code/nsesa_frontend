...
import { Menu, ReceiptText, ClipboardList, LogOut, X, LayoutDashboard, Users, Package } from "lucide-react"
...
function getNavItems(role: string): NavItem[] {
  const workerItems: NavItem[] = [
    ...
    { label: "Transactions", href: "/dashboard/worker/transactions", icon: ClipboardList },
  ]
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="px-6 py-5">
        <span className="text-xl font-bold text-green-600">Nsesa</span>
      </div>
      ...
      <div className="px-4 py-4 space-y-3">
        <div className="px-3">
          ...
          <p className="text-xs text-zinc-500 capitalize">{session.user.worker_role}</p>
        </div>
        <button
  ...
  return (
    <div className="flex h-screen bg-zinc-50">
      ...
      <aside className="hidden md:flex w-60 shrink-0 flex-col border-r border-zinc-200 bg-white">
        <SidebarContent session={session} navItems={navItems} pathname={pathname} />
      </aside>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex md:hidden items-center gap-3 border-b border-zinc-200 bg-white px-4 py-3">
          ...
          <span className="text-lg font-bold text-green-600">Nsesa</span>
        </header>
